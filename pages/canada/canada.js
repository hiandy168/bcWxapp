...
Page({
  ...
  getTheNewOneNoOpenData : function(that){ // 获取最近一站的未开信息
    ...
    wx.request({
        url: app.globalData.http_header + 'index.php?m=Gamematch&a=index&status=0&appjson=1&gameid=2&status=1&rows=1&sort=id&order=asc', 
        //status 0 未开奖 1 开奖
        data: {},
        method: 'GET', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
        // header: {}, // 设置请求的 header
        success: function(res){
          ...
          that.data.interval = setInterval(function (){ 
            ...
            if(delta < that.data.ttime || delta > that.data.opentime){ //判断是否在下注时间区
  ...
  getTheNewOneOpenedData :function (that){ // 获取最新的已开启的一站
     ...
     wx.request({
      url: app.globalData.http_header + 'index.php?m=Gamematch&a=index&appjson=1&gameid=2&status=3&rows=1', 
      ...
      success: function(res){
        ...
        // 判断是否已经开启最新一站
        if( that.data.latestData.length != 0){ // 数据为空
  ...
  getWinInfo : function (that,issue) { // 获取最新一站的情况
    ...
    wx.request({
      url: app.globalData.http_header + '/index.php?m=Gamebet&a=index&appjson=1&gameid=2&issue='+ issue +'&com=' + com, 
      data: {},
      method: 'GET', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
      // header: {}, // 设置请求的 header
      success: function(res){
        ...
        // that.isDebug(that,res.data.debug)
  ...
  getThe10Datas : function(that){ // 获取最近的几站数据 
  that.setData({loading: true})
    wx.request({
      url: app.globalData.http_header + 'index.php?m=Gamematch&a=index&appjson=1&gameid=2&status=3&rows='+that.data.pageSize+'&page='+that.data.currentPage, // gameid:北京 加拿大 status: 1开奖 0未开奖 rows 页面大小 page 页数
      data: {},
      method: 'GET', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
      // header: {}, // 设置请求的 header
      success: function(res){
  ...
  onUnload:function(){
    ...
    // 清除时间栈
    if(that.data.nodata){
      clearInterval(that.data.nodata)
    }
  },
  ...
  formSubmit : function(e){ // 发送消息
    ...
    if(that.data.loading){
      ...
      msg.set({
          ...
          to: wx.getStorageSync('canadagroupid'),
          roomType: false,
          chatType: 'chatRoom',
          ext : {
            user : user
  ...
  manalFormSubmit : function(that,e,avt,type1){ // 发送消息
    if(e.detail.value.value.length > 0){  // 有文字
      ...
      var textHeight = tool.getTextWidth(e.detail.value.value, (750 - 60)*0.8*0.8 - 36, 28, 32)
      var user =  wx.getStorageSync('userInfo')
      user.avatarUrl = avt ? avt : '../../images/default.png'
  ...
  bindscrolltolower : function(e){
    var that = this
    wx.request({
      url: app.globalData.http_header + 'index.php?m=Gamematch&a=index&appjson=1&gameid=2&status=3&rows='+that.data.pageSize+'&page=' + that.data.currentPage,
      data: {},
      method: 'GET', // OPTIONS, GET, HEAD, POST, PUT, DELETE, TRACE, CONNECT
      // header: {}, // 设置请求的 header
      success: function(res){
